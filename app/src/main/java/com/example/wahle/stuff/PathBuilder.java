...
public class PathBuilder {
    ...
    public static Path buildPathFromWaypoints(List<Waypoint> w, String ball) {
        Path p = new Path();
        if (w.size() < 2)
        ...
        if (w.size() > 2) {
            do {
                new Arc(getPoint(w, i), getPoint(w, i + 1), getPoint(w, i + 2)).addToPath(p, ball);
                i++;
            } while (i < w.size() - 2);
        }else if (w.size() == 2) {
            new Line(w.get(w.size() - 2), w.get(w.size() - 1)).addToPath(p, ball);
        }
        return p;
    }
